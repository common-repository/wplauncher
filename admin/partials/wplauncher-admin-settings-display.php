...
<div class="wrap">
		        ...
		        <h2>WPLauncher Settings</h2>  
		         <!--NEED THE settings_errors below so that the errors/success messages are shown after submission - wasn't working once we started using add_menu_page and stopped using add_options_page so needed this-->
				<?php settings_errors(); ?>  
				<?php /*active tab variable set in the class-wpmerchant-admin.php*/ ?>
		        <h2 class="nav-tab-wrapper">  
		            <a href="?page=wplauncher-settings&tab=general" class="nav-tab <?php echo $active_tab == 'general' ? 'nav-tab-active' : ''; ?>">General</a>  
		            <!--a href="?page=wplauncher-settings&tab=payment" class="nav-tab <?php echo $active_tab == 'payment' ? 'nav-tab-active' : ''; ?>">Payment</a> 
					<a href="?page=wplauncher-settings&tab=emails" class="nav-tab <?php echo $active_tab == 'emails' ? 'nav-tab-active' : ''; ?>">Email List</a> 
		        ...
		        <form method="POST" action="options.php">  
		            ...
		            if( $active_tab == 'general' ) {  
		                settings_fields( 'wplauncher_general_settings' );
		                do_settings_sections( 'wplauncher_general_settings' ); 
